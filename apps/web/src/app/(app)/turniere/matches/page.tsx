...
export default async function AlleMatchesPage(): Promise<React.JSX.Element> {
  ...
  const matches =
    turnierIds.length > 0 ? await tournaments.listMatches({ tournamentIds: turnierIds, limit: 300 }) : [];

  const dringend = matches.filter(
    (match) => match.status === 'DISPUTED' || match.status === 'LIVE' || match.status === 'AWAITING_RESULT',
  );
    ...
    <>
      ...
      {dringend.length > 0 ? (
        <section className="space-y-3">
          ...
          <MatchAdminList matches={dringend.map((match) => alsZeile(match, match.tournament.name))} />
        </section>
      ) : null}

      <section className="space-y-3">
        <h2 className="text-sm font-semibold">{dringend.length > 0 ? 'Übrige Matches' : 'Alle Matches'}</h2>
        <MatchAdminList
