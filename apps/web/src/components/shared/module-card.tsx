...
export function ModuleCard({ name, description, icon, enabled, href }: ModuleCardProps): React.JSX.Element {
  ...
  const content = (
    <div
      className={cn(
        // `min-w-0`: als Rasterkind ist die Mindestbreite sonst die des
        // Inhalts. Ein langer Modulname passte dann nicht in seine Spalte,
        // die Spalte wuchs mit - und das ganze Raster schob die Seite ueber
        // den rechten Rand hinaus. Zusammen mit `break-words` am Namen bricht
        // stattdessen das Wort.
        'flex h-full min-w-0 flex-col items-center gap-2 rounded-xl border border-border bg-card/60 px-4 py-5 text-center transition-colors',
