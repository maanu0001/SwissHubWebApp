...
import { blockMemberAction, liftBlockAction } from '@/modules/tournaments/admin-actions';
 ...
 * Turniersperren.
 *
 * Eine Sperre verhindert neue Anmeldungen. Laufende Turniere bleiben
 * unberuehrt - wer jemanden aus einem laufenden Turnier nehmen will,
 * disqualifiziert dort, und das ist eine andere Entscheidung.
 */
export function TournamentBlockManager({
  csrfToken,
  sperren,
}: {
  csrfToken: string;
  ...
  async function sperren_(): Promise<void> {
    ...
    if (antwort.ok) {
      toast.success(`${auswahl.username} kann sich zu keinem Turnier mehr anmelden.`);
      setAuswahl(null);
      setGrund('');
      setTage(0);
      router.refresh();
    ...
    <>
      ...
      >
        <MemberPicker
          ...
          value={auswahl}
          onChange={setAuswahl}
          label="Mitglied von Turnieren ausschliessen"
        />
        ...
        <div className="flex justify-end">
          <Button
            type="submit"
            variant="outline"
            disabled={auswahl === null || grund.trim().length < 5 || laeuft !== null}
          >
            {laeuft === 'create' ? (
      ...
      {sperren.length === 0 ? (
        <EmptyState
          title="Keine Sperren"
          description="Niemand ist derzeit von der Turnierteilnahme ausgeschlossen."
        />
      ) : (
